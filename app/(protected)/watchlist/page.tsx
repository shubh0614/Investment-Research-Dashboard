...
import { Bookmark, Plus, Trash2, Loader2, Search } from "lucide-react";
...
export default function WatchlistPage() {
  ...
  const addMut = useMutation({
    ...
    onError: (e) => setFormErr(e instanceof Error ? e.message : "Failed to add"),
  });
  ...
  return (
    <div className="mx-auto w-full max-w-2xl px-8 py-10" style={{ color: "var(--text)" }}>
      <div className="mb-6">
        <h1 className="text-2xl font-semibold" style={{ color: "var(--text)", letterSpacing: "-0.018em" }}>
          Watchlist
        </h1>
        <p className="mt-0.5 text-sm" style={{ color: "var(--text-muted)" }}>
          Track companies for quick research access
        </p>
      </div>
      ...
      <form
        onSubmit={handleAdd}
        className="mb-6 rounded-xl border p-5 reveal"
        style={{ background: "var(--surface)", border: "1px solid var(--border)" }}
      >
        <p className="mb-3 text-sm font-medium" style={{ color: "var(--text)" }}>Add a ticker</p>
        <div className="flex gap-3">
          <input
            ...
            placeholder="NVDA"
            maxLength={10}
            className="h-9 w-24 rounded-lg border px-3 font-mono text-sm uppercase outline-none transition-colors duration-150"
            style={{ background: "var(--surface-2)", border: "1px solid var(--border)", color: "var(--text)", fontVariantNumeric: "tabular-nums" }}
          ...
          <input
            ...
            className="h-9 flex-1 rounded-lg border px-3 text-sm outline-none transition-colors duration-150"
          ...
          <button
            type="submit"
            disabled={addMut.isPending || !ticker.trim() || !company.trim()}
            className="flex h-9 items-center gap-1.5 rounded-lg px-4 text-sm font-medium disabled:opacity-50"
        ...
        {formErr && (
          <p className="mt-2 text-xs" style={{ color: "var(--negative)" }}>{formErr}</p>
        )}
      </form>
      ...
      {!isLoading && items.length > 0 && (
        <div
          className="overflow-hidden rounded-xl border stagger"
          style={{ background: "var(--surface)", border: "1px solid var(--border)" }}
        >
          {items.map((item, i) => (
            <div
            ...
            >
              <span
                className="w-16 font-mono text-sm font-semibold"
              ...
              <Link
                ...
                className="flex h-7 items-center gap-1 rounded-lg px-2.5 text-xs transition-colors duration-150"
